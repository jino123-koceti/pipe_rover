...
void Preprocess::oust64_handler(const sensor_msgs::msg::PointCloud2::UniquePtr &msg)
{
  ...
  else
  {
    ...
    // COIN-LIO photometric: point_filter_num을 무시하고 전체 포인트 사용
    // 원본 COIN-LIO와 동일하게 raw index를 normal_x에 저장
    for (int i = 0; i < (int)pl_orig.points.size(); i++)
    {
      ...
      // COIN-LIO: raw cloud index → projector lookup, range → occlusion check
      added_pt.normal_x = i;
