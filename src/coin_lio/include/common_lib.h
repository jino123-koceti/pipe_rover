...
// COIN-LIO: Pose6D struct (replaces ROS message)
struct Pose6D {
    double offset_time;
    double acc[3];
    double gyr[3];
...
// COIN-LIO: LidarFrame for intensity image processing
struct LidarFrame {
...
// Sub-pixel interpolation for intensity images
template<typename T>
inline double getSubPixelValue(const cv::Mat& img, double u, double v) {
    int u0 = (int)u;
    int v0 = (int)v;
    double du = u - u0;
    double dv = v - v0;
    if (u0 < 0 || u0 >= img.cols - 1 || v0 < 0 || v0 >= img.rows - 1) return 0;
    return (1-du)*(1-dv)*img.ptr<T>(v0)[u0] + du*(1-dv)*img.ptr<T>(v0)[u0+1] +
           (1-du)*dv*img.ptr<T>(v0+1)[u0] + du*dv*img.ptr<T>(v0+1)[u0+1];
}
