...
class FeatureManager {
 public:
    FeatureManager(rclcpp::Node::SharedPtr nh, std::shared_ptr<Projector> projector);

    void updateFeatures(const LidarFrame& frame, const std::vector<V3D>& V, const M4D& T_GL);

    const std::vector<PhotoFeature>& features() const { return features_; }
 ...
 private:
    void loadParameters(rclcpp::Node::SharedPtr nh);
    ...
    rclcpp::Publisher<visualization_msgs::msg::MarkerArray>::SharedPtr moment_pub;
    rclcpp::Node::SharedPtr node_;
    std::vector<PhotoFeature> features_;
